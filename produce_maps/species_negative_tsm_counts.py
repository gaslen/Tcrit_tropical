...
for year in ['2001', '2020', '2050', '2100']:
    ...
    if year == '2050' or year == '2100':
        outfile_sps_neg_tsm_low_acclim_counts = (DATA_PATH + f"/outputs/species_negative_tsm_counts_acclim_038_map_{year}{version}.npy")
        outfile_sps_neg_tsm_high_acclim_counts = (DATA_PATH + f"/outputs/species_negative_tsm_counts_acclim_049_map_{year}{version}.npy")

    if year == '2050' or year == '2100':
         if exists(outfile_sps_counts) and exists(outfile_sps_neg_tsm_counts) and exists(outfile_sps_neg_tsm_low_acclim_counts) and exists(outfile_sps_neg_tsm_high_acclim_counts):
            continue
    else:
        if exists(outfile_sps_counts) and exists(outfile_sps_neg_tsm_counts):
            continue
    ...
    if year == '2050' or year == '2100':
        sps_neg_tsm_low_acclim_counts = np.zeros(m.shape)
        sps_neg_tsm_high_acclim_counts = np.zeros(m.shape)
    ...
    for sp in tqdm(list_species, total=len(list_species)):
        ...
        if year == '2050' or year == '2100':
            sps_neg_tsm_low_acclim_counts += np.where((sp_tcrit*sp_map + (delta_tsurf[slice_tropics]*0.38) <= m), 1, 0)
            sps_neg_tsm_high_acclim_counts += np.where((sp_tcrit*sp_map + (delta_tsurf[slice_tropics]*0.49) <= m), 1, 0)
        
    np.save(outfile_sps_counts, sps_counts)
    np.save(outfile_sps_neg_tsm_counts, sps_neg_tsm_counts)
    if year == '2050' or year == '2100':
        np.save(outfile_sps_neg_tsm_low_acclim_counts, sps_neg_tsm_low_acclim_counts)
        np.save(outfile_sps_neg_tsm_high_acclim_counts, sps_neg_tsm_high_acclim_counts)
